...
<nav class="sidebar" style="margin-top:-16px;">
  <ul>
    ...
    <li class="menu-item-profile {{ preg_match('/add/photo'.Auth::id(), URL::current()) ? 'menu-item-profile-active' : '' }}"><a href="{{ URL::route('add') }}">Добавить изображение</a></li>
    <li class="menu-item-profile {{ preg_match('/edit/user'.Auth::id(), URL::current()) ? 'menu-item-profile-active' : '' }}"><a href="{{ URL::route('edit') }}">Редактировать профиль</a></li>
    <li class="menu-item-profile {{ preg_match('/liked/photo'.Auth::id(), URL::current()) ? 'menu-item-profile-active' : '' }}"><a href="{{ URL::route('liked') }}">Избранное</a></li>
